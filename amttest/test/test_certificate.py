...
class TestExam(BaseTest):
    ...
    def test_get_certificate(self):
        ...
        self.assert200(no_user_resp, 'no user should still return an empty list')
        self.assertEqual(no_user_resp.json, [], 'if user does not exist, response should return an empty list')
    ...
    def test_update_certs(self):
        ...
        response_no_data = self.client.post('amttest/api/certificate/1/1', headers=self.header_dict)
        self.assert400(response_no_data, 'some data is required for a new entry')

        response_no_exam = self.client.post('amttest/api/certificate/1/42', headers=self.header_dict, data=json.dumps(['abc', '123']))
        self.assert400(response_no_exam, 'if exam doesnt exist, it cant be graded')
        ...
        response_short_payload = self.client.post('amttest/api/certificate/1/1', headers=self.header_dict, data=json.dumps(short_payload))
        self.assert400(response_short_payload, 'There must be equal answers to active questions from each section')
        ...
        response_bad_question = self.client.post('amttest/api/certificate/1/1',
        ...
        response_bad_answer = self.client.post('amttest/api/certificate/1/1',
                                                headers=self.header_dict,
                                                data=json.dumps(payload_bad_answer))
        ...
        self.assertEqual(response_correct.status_code, 201, 'grading should return 201')
        self.assertTrue(response_correct.json['passed'], 'grading should give a pass with all correct')
        self.assertEqual(response_correct.json['correct'], 4, 'all questions should be correct')
        self.assertEqual(response_correct.json['possible'], len(payload), 'each question should be graded, all questions are the possible score')
        question = Question.query.filter_by(questionid=1).first()
        self.assertEqual(question.correct, 1, 'stats should update for getting a question correct')
        self.assertEqual(question.used, 1, 'stats should flag a question as being used')
        answer = Answer.query.filter_by(answerid=3).first()
        self.assertEqual(answer.chosen, 1, 'stats should update answers for being used')
        cert_pass = Certificate.query.filter_by(certid=response_correct.json['certid']).first()
        cert_dict = table2dict(cert_pass)
        cert_dict.pop('testdate')
        return_dict = response_correct.json.pop('testdate')
        ...
        self.assertEqual(response_wrong.status_code, 201, 'even wrong answers should return a 201')
        self.assertFalse(response_wrong.json['passed'], 'failed tests should fail')
    ...
    def enter_data(self):
        ...
        data.append(Answer(
            ...
            correct=False, questionid=3))
        data.append(Answer(
            answer='No hits count and the striking player should remove hiimself from the field',
            correct=False, questionid=3))
        data.append(Answer(
            answer='All hits count, given the struck and striking players continue play, otherwise combat should be reset to before the strike',
            correct=True, questionid=3))
        data.append(Question(
            question='When should "Safety" be called?',
            sectionid=2))
        data.append(Answer(
            answer='When there is an injured player, or obstruction on the field',
            correct=True, questionid=4))
        data.append(
            Answer(answer='Never, "Hold" should be used instead', correct=False,
                   questionid=4))
        data.append(
            Answer(answer='When there is a dispute of the rules', correct=False,
                   questionid=4))
        data.append(Answer(
            answer='When a player feels that the game is unfair and wishes to leave',
            correct=False, questionid=4))
