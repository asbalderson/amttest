...
class TestUser(TestCase):
    ...
    def test_get_all_users(self):
        response_empty = self.client.get('amttest/api/users')
        ...
        response = self.client.get('amttest/api/users')
        ...
        user1.archive = True
        db.session.commit()
        response_archive = self.client.get('amttest/api/users')
    ...
    def test_add_user(self):
        response_no_header = self.client.post('amttest/api/users')
        self.assert400(response_no_header, 'post should require a token')

        response_no_data = self.client.post('amttest/api/users', headers=self.header_dict)
        ...
        new_user = self.client.post('amttest/api/users', data=json.dumps(data), headers=self.header_dict)
        self.assertEqual(new_user.status_code, 201, 'post response returned no data')

        repete_user = self.client.post('amttest/api/users', data=json.dumps(data), headers=self.header_dict)
        ...
        missing_data = self.client.post('amttest/api/users', data=json.dumps(data), headers=self.header_dict)
        ...
        extra_data = self.client.post('amttest/api/users', data=json.dumps(data), headers=self.header_dict)
    ...
    def test_put_user(self):
        response_no_header = self.client.put('amttest/api/users/42')
        self.assert400(response_no_header, 'post should require a token')

        response_no_data = self.client.put('amttest/api/users/42', headers=self.header_dict)
        self.assert400(response_no_data, 'some data is required for a new user')
        ...
        response_no_user = self.client.put('amttest/api/users/42', data=json.dumps(update_data),
                                           headers=self.header_dict)
        self.assert400(response_no_user, 'should return 400 when user does not exist')

        basic_update = self.client.put('amttest/api/users/1', data=json.dumps(update_data), headers=self.header_dict)
        ...
        update_bad_data = self.client.put('amttest/api/users/1', data=json.dumps(dummy_data), headers=self.header_dict)
    ...
    def test_delete_user(self):
        response_no_header = self.client.delete('amttest/api/users/42')
        self.assert400(response_no_header, 'delete should require a token')

        response_empty = self.client.delete('amttest/api/users/42', headers=self.header_dict)
        ...
        response_delete = self.client.delete('amttest/api/users/1', headers=self.header_dict)
